...
Graph newGraph(int n){
   ...
   G->array = calloc(n, sizeof(List)); // XXX May cause memory errors
...
int getSource(Graph G){
   ...
   if(G->bfsCalled == 1){
      return G->source; // XXX needs value
   }
   return NIL;
}
...
int getParent(Graph G, int u){
   ...
   if(G->bfsCalled == 1){
      return G->parent[u-1]; // XXX needs value
   }
   return NIL;
}
...
void addArc(Graph G, int u, int v){
   ...
   G->size++;
   G->bfsCalled = 0; // XXX unsure if needed
}
